...
_README_MODEL_MAP = {
    ...
    'grudeva2025.reduced': {"stage": 'Extraction', "role": 'Registered scientific check — no common-scenario adapter yet', "physics": "A reduced asymptotic extraction model with a moving wetting front and fine/coarse particle families (saturated-fines regime; prescribed flow). Its code is a faithful port of the authors' solver, used under **Dr. Yoana Grudeva's direct written permission, 2026-08-14**, covering code and data ([#73](https://github.com/trbrewer/puckworks/issues/73), `PERMISSION_DOCUMENTED`); **no SPDX licence** — not MIT/CC-BY/public-domain ([permission](docs/permissions/grudeva2025.md), [notices](THIRD_PARTY_NOTICES.md)). Runs its **registered gates**; not a common-scenario lens — its grain-volume basis has no tested conversion to bed-volume (technical, not rights). Evidence unchanged."},
    ...
    'maille2024.phi_closure': {"stage": 'Grind', "role": 'Calibration / comparison', "physics": 'Predicts from a measured particle size distribution what fraction of the soluble material releases fast — fines plus a thin shell on each coarse particle. Instrument-specific (186 µm cut); coarse grinds only, so espresso is an extrapolation.'},
    'maille2024.two_regime': {"stage": 'Extraction', "role": 'Calibration / comparison', "physics": 'Early-time release of five named compounds in a stirred batch vessel, as a fast phase plus a slow phase. The best sub-30-second per-compound data here, but with no bed, pressure or flow, and no yield or strength output.'},
...
_CONTENT = {
    ...
    'grudeva2025.reduced': (
        'a reduced whole-shot extraction model with a moving wetting front — its own registered checks run, not your recipe.',
        "This model tracks the wetting front sweeping the dry puck and the saturated plateau that follows, which is why first drip and the early part of a shot behave the way they do. Its code is a port of the authors' own solver, used and shared here with the author's direct written permission of 2026-08-14 covering code and data (no formal open-source licence was granted — see issue #73). What runs here are its registered checks against its own reference data, not a simulation of your shot: its concentrations are measured per grain volume, which cannot be laid alongside the bed-volume yield the rest of the tour uses without a conversion nobody has tested yet."),
    'lee2023.feedback': (
    ...
    'maille2024.phi_closure': (
        'what share of a coffee dissolves quickly, worked out from the particle sizes alone.',
        'Some of a shot comes out almost immediately — from the smallest particles and from the outer skin of the larger ones — and the rest has to diffuse out slowly. This estimates that quick-release share from a grind measurement rather than fitting it to a taste result, which is why grinding finer usually pushes more of the cup into the fast phase. Two cautions: it was measured on coffee much coarser than espresso, so at espresso grind it is being read outside its tested range; and its "fast fraction" counts something different from other models that use the same phrase, so the numbers are not interchangeable.'),
    'maille2024.two_regime': (
        'how fast five individual coffee compounds come out of the grounds in the first half-minute.',
        'Caffeine, chlorogenic acid and three fruit acids do not all leave the grounds at the same rate, and the first thirty seconds is where most of that difference happens — which is part of why very short and very long shots taste different, not merely stronger or weaker. This was measured by stirring grounds in a large vessel of water, with no puck, no pressure and no flow, so it is a clean look at the chemistry timing rather than a model of your machine; it also produces no strength or yield figure at all.'),
