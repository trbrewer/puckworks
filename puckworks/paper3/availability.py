...
def render_implementation_status(s: dict) -> str:
    out = [
        ...
        "",
        f"**Table 1h. Implementation status.** {s['n_capabilities']} capabilities. This table is "
...
def render_matrix(m: dict) -> str:
    ...
    out = [
        "<!-- generated by puckworks.paper3.availability — do not edit by hand -->",
        "",
        f"**Table 1f. Availability matrix.** {m['n_components']} registered components, "
    ...
    out += ["", "**Table 1g. Availability by component.** Every registered component against "
            "each dimension, with its blocking reason where one applies.", "",
...
def main(argv=None) -> int:
    ...
    if "--splice" in argv:
        # Regenerate the committed artifacts FIRST. This branch used to return before `--write`
        # was handled, so `--write --splice` silently did only the splice and left
        # availability_matrix.md / implementation_status.md stale -- caught only by the
        # freshness test, on a clean checkout. Flags must not be order-dependent.
        write()
        problem = splice(write_it=True)
        print(problem or "artifacts written and manuscript tables spliced")
