...
class Defect:
    ...
    #: WHICH PATH the case actually exercises. "executable" lumped an end-to-end contract
    #: violation together with a manuscript phrase sentinel, which made "15 executable mutations
    #: that perturb a real input and run the production guard" too broad a claim (fourth review
    #: P0-12). The four values describe the path, and `is_control` remains a separate axis, so a
    #: control is never counted as an injected defect regardless of the path it runs through.
    #:
    #:   production_path_mutation      -- mutates a real input and runs the production guard
    #:                                    (contracts, registry, public schema, evidence graph)
    #:   integration_regression_sentinel -- runs a real repository tool, but over manuscript text
    #:   static_manuscript_check       -- compares strings between documents; no production code
    #:   limitation_analysis           -- reasoned, not executed: the guard provably cannot see it
    execution_type: str = "production_path_mutation"
...
CORPUS = [
    ...
    Defect("D07", "Section renumbered, cross-reference left stale", "prose_drift",
           ...
           independence_group="manuscript_sentinel", severity="low", execution_type="integration_regression_sentinel"),
    ...
    Defect("D09", "Retired overclaim phrase returns in the venue conversion", "evidence",
           ...
           independence_group="manuscript_sentinel", severity="medium", execution_type="integration_regression_sentinel"),
    Defect("D10", "Headline number edited in prose only", "provenance",
           ...
           independence_group="number_provenance", severity="high", execution_type="static_manuscript_check"),
...
# runner
# --------------------------------------------------------------------------------------------
#: Manuscript block markers for the generated defect-class table.
_T6A_BEGIN = "<!-- table6a:begin -->"
_T6A_END = "<!-- table6a:end -->"


def render_table6a(result=None) -> str:
    """Table 6a, generated from `run_benchmark()`.

    Fourth review P0-1: the manuscript hand-maintained these cells and two rows had gone stale --
    observable_semantics printed 2/3 against the benchmark's 4/4, and unit printed 2/4 against 1/3.
    The unit row reintroduced exactly the denominator error the surrounding text says was
    corrected, by counting a passing CONTROL as an injected defect. Controls are specificity
    checks; they are reported in their own column and never inside the injected denominator.
    """
    r = result or run_benchmark()
    lines = [_T6A_BEGIN,
             "<!-- GENERATED by `python -m puckworks.paper3.defect_injection --splice`. "
             "DO NOT EDIT BY HAND. -->",
             "",
    ...
    for k, v in sorted(r["by_family"].items()):
        passed = v["controls"] - v["false_positives"]
        lines.append(f"| {k} | {v['true_positives']} / {v['defects']} | "
                     f"{passed} / {v['controls']} |" if v["controls"]
                     else f"| {k} | {v['true_positives']} / {v['defects']} | — |")
        td += v["true_positives"]; ti += v["defects"]
        tc += v["controls"]; tcp += passed
    lines.append(f"| **Total** | **{td} / {ti}** | **{tcp} / {tc}** |")
    lines += ["", _T6A_END]
    return "\n".join(lines)


def splice_table6a(write_it: bool = True) -> list:
    """Splice Table 6a into the manuscript. Returns problems (empty = current)."""
    import pathlib as _pl
    man = _pl.Path(__file__).resolve().parents[2] / "docs" / "PAPER_3_PUCKWORKS_DRAFT.md"
    text = man.read_text(encoding="utf-8")
    if _T6A_BEGIN not in text or _T6A_END not in text:
        return ["table6a: markers missing from the manuscript"]
    block = render_table6a()
    current = _T6A_BEGIN + text.split(_T6A_BEGIN, 1)[1].split(_T6A_END, 1)[0] + _T6A_END
    if current == block:
        return []
    if not write_it:
        return ["table6a: inline block is STALE -- run --splice"]
    man.write_text(text.split(_T6A_BEGIN)[0] + block + text.split(_T6A_END, 1)[1],
                   encoding="utf-8")
    return []
...
def run_benchmark():
    ...
    executable = [r for r in defects if r["execution_type"] == "production_path_mutation"]
    ...
    return dict(
        ...
        by_execution_type={k: sum(1 for r in defects if r["execution_type"] == k)
                           for k in sorted({r["execution_type"] for r in defects})},
        by_execution_type_detected={
            k: sum(1 for r in defects if r["execution_type"] == k and r["caught"])
            for k in sorted({r["execution_type"] for r in defects})},
...
if __name__ == "__main__":                                     # pragma: no cover
    ...
    if "--splice" in sys.argv:
        problems = splice_table6a(write_it=True)
        print("\n".join(problems) if problems else "Table 6a spliced")
        sys.exit(1 if problems else 0)
    if "--verify" in sys.argv:
        problems = splice_table6a(write_it=False)
        print("\n".join(problems) if problems else "Table 6a is current")
        sys.exit(1 if problems else 0)
    res = run_benchmark()
