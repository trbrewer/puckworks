...
| Gate-level evidence graph (WP2.4, schema v2) | **CI-verified (draft) + paper3-strict + all-strict green; roll-up policy adopted** | `paper3.evidence_graph`; `EVIDENCE_LINKS.json`; `tests/test_evidence_graph.py` | **49/49 adjudicated**; both `--strict --scope paper3` (20 asserted claims) and `--strict --scope all` green in `paper3-evidence.yml` (NEITHER yet a required branch check — user promotes). Component→gate roll-up policy adopted + enforced; `foster2025.infiltration` demoted `controlled_independent`→`sign_or_compatibility` (ROADMAP §7.1). Registry now declares NO `controlled_independent` component. |
