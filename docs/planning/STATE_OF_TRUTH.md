...
| Gate-level evidence graph (WP2.4, schema v2) | **CI-verified (draft) + paper3-strict green** | `paper3.evidence_graph`; `EVIDENCE_LINKS.json`; `tests/test_evidence_graph.py` | 23/49 adjudicated (16 v2 landing + 7 PR-C priority gates); `--strict --scope paper3` green over 10 asserted claims — MAY now be promoted to a required check; 26 drafts remain for `--scope all` (manual) |
