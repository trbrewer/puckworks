...
| Gate-level evidence graph (WP2.4, schema v2) | **paper3-strict + all-strict REQUIRED-CHECK green; roll-up + zero-gate policies enforced** | `paper3.evidence_graph`; `EVIDENCE_LINKS.json`; `tests/test_evidence_graph.py` | **51/51 adjudicated** (added `gate_streamtube_heldout` + `gate_pack_generator_admissible`); both `--strict --scope paper3` (22 asserted) and `--scope all` are **required branch checks** (user enabled). Roll-up policy enforced (`foster2025.infiltration` demoted → `sign_or_compatibility`, ROADMAP §7.1; no `controlled_independent` component). Zero-gate policy enforced: streamtube + pack_generator now gate-backed; `lb_taichi` is the sole acknowledged zero-gate exception (out-of-CI Taichi cross-check). |
