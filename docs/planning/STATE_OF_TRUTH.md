...
| pv05-model-composition | PV-05 'More Physics Made It Worse' — model-composition interactive | public-value | P0 | maintainer | Merge the PV-05 product PR (public.model_composition producer-bound data contract + static four-scene model-composition interactive + tests), deploy to GitHub Pages, verify the live site at trbrewer.github.io/puckworks/model-composition/, then activate (README link + landing card + status flip to complete). | Every displayed number generated by kappa_t_ladder / degeneracy_rmse / composition_residual / simulate and bound to the CC-BY Waszkiewicz trace; extraction-only 0.116 < constant 0.573 < composite 0.648 g/s; shared porosity over-closes to 0.0998; badge EXPLORATORY_SIMULATION, strength qualitative; accessible static site with text+image equivalents; strict CSP, no external network/tracking; rejects THIS composition, not swelling in general; not coupled into Guided Pull. |

## Proposed (next up)

| id | title | area | priority | owner |
|---|---|---|---|---|
| pv04-analysis-autopsy | PV-04 'How We Falsified Our Own Espresso Headline' — analysis autopsy | public-value | P0 | maintainer |
