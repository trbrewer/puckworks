...
| guided-pull-laboratory | PV-19B: Guided Pull Laboratory — compatible-model lenses + full registry exposure | product | P1 | maintainer | Add genuinely executed native reference runners for low-risk components, and a second common-scenario adapter only if a bounded conversion can be demonstrated without inventing a grinder/unit/reference-volume mapping. | Core + contract hardening landed (development 0.4.0.dev0). puckworks/product/lab.py: exact scenario identity + override provenance (typed ScenarioRequest/ScenarioExecution; guided_v1 runs no longer mislabelled pv19_named); comparison schema v3; separated scientific-payload vs full-artifact integrity hashes (downloadable artifact carries source_commit/package_version/workflow_run_id/wheel_sha256, supplied explicitly - no git subprocess in the producer); correct observable roles (prescribed/derived/simulated/unsupported/diagnostic); explicit 25-component capability/rights matrix (grudeva2025.reduced RIGHTS_BLOCKED pending #73); honest reference-suite coverage (executed vs runner-not-implemented); real scientific trace plots in Streamlit + Actions batch (required figure) + hash manifest. ONE executed common-scenario lens (cameron2020.extraction_bdf via simulate_pull; no equation duplication). Streamlit app verified booting locally (/_stcore/health 200). Not a validated digital twin; no evidence upgrade. Three genuinely-executed native reference runners now ship (waszkiewicz static refit, wadsworth permeability collapse, foster infiltration first-drip bracket) via puckworks/product/lab_runners.py, each reusing gate callables (no equation duplication), interactive-fast, failure-isolated. A demonstrated second common-scenario adapter remains (see #70); Grudeva blocked by rights (#73). |
