...
| **I-024** — can one transport state explain every measured species at once? | snapshot `c1b7d79`, screened on `insights/if5-wave1-cheap-screens` | Giving every species its own transport rate buys **nothing** held out. On the angeloni2023 campaign, held out at the interior 9-bar conditions, `Z_independent / Z_shared` = 1.009 and 1.001 at the two ends of the source's stated replicate-RSD band — against a 0.70 threshold. A per-species inventory **level** absorbs 76.1 % of the raw residual, and the fitted levels recover angeloni's own Table 7 inventory better than pannusch's does: the species differ in how much of them is in the coffee, not in how they move through it. At the conservative end the between-species spread also falls below 1 σ, i.e. the residual is shared, not species-specific. | [`screens/I-024/`](screens/I-024/) | **(a)** A **fraction-resolved** species dataset for this campaign. The cup-integrated endpoint provably discards the rate information (`ANALYSIS_transfer` positive control); a 25× spread in fitted rate produced no held-out separation here. Timed fractions would be a different — and answerable — question. **(b)** Solute-specific replicate RSD for caffeine / trigonelline / CGA (already recorded as owed in `angeloni2023/MANIFEST_UNCERTAINTY.md`) would let C1 and C2 be evaluated rather than bracketed — though it cannot change C3, which is scale-free. **Not** reopened by a claim that species kinetics differ in the literature: this screen bounds what *this campaign at this observable* can see. |
