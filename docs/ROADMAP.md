...
| angeloni2023 Table 7 inventories **(intake PARTIAL)** + Tables 2–5 66-shot chemistry **(pending drop)** | angeloni2023 | transcribe from article tables (no repo; MDPI Cloudflare-blocked for fetch) | first **INDEPENDENT** multi-species validation target for **pannusch2024** (currently post-fit only); "multi-class solute chemistry" backlog | independent (different machine/coffee/basket than pannusch's fit) | **2** — Table 7 done; 66-shot bioactives/TS/lipids need a Tim drop |
