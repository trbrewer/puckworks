...
| angeloni2023 Tables 1–7 (66-shot chemistry + inventories) **(intake DONE)** | angeloni2023 | transcribed from article tables (Tim xlsx drop; no repo) | first **INDEPENDENT** multi-species validation target for **pannusch2024** (currently post-fit only); "multi-class solute chemistry" backlog | independent (different machine/coffee/basket than pannusch's fit) | **done** — bioactives (66×11) + TS + lipids + inventories loaded; bracket gate shows Cameron reads low (2nd dataset after egidi) |
