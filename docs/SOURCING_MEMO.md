...
| G10 | coffee-extract mu,rho vs T,TDS (Telis-Romero) | source_curve_reproduction for the EXTRACT rheology (espresso still extrapolated) | independent espresso-TDS measurement (only remaining item; would lift the composition caveat) | CLOSED-NEGLIGIBLE 2026-07-15 — full Table 1/2 digitized (51 measured cells, closures reproduce at authors' fit quality); μ(c,T) sensitivity study (`analysis.g10_viscosity_sensitivity`) shows ≤~3% shot-integrated Darcy error → NO runtime hook. Composition + dilute-end caveats stand |
