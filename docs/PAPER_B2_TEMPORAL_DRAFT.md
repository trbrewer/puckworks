...
| Empirical $\Phi(t)$ | 0.116 | temporal candidate; no coefficient fitted to this flow trace, but its temporal input is partly derived from the same flow (§4.3) |
...
The four-parameter cubic reaches 0.096 g s⁻¹. Its fit is at least as close as the mechanistic trajectory, so the reconstruction does not identify the poroelastic–dissolution closure. Instead, the non-trivial result is that a same-campaign, target-informed temporal trajectory nearly reaches the same-trace descriptive benchmark without fitting an additional coefficient to the scored flow trace (its temporal input, however, is partly built from that same flow; §4.3).
...
These results rule out neither swelling nor fines migration as constituents of the real bed. They show only that, in the tested fixed-pressure isolation, those resistance-increasing branches cannot be the sole positive contribution. Two candidate mechanisms carry the required net sign for a rising fixed-pressure flow: dissolution-linked porosity opening, and a decline in liquor viscosity as the dissolved-solids concentration falls over the shot. Only the first is a scored branch in the quantitative ladder; the liquor-viscosity effect remains **outside** that ladder (§6) and is discussed here as a qualitative sign candidate, not an implemented, scored reconstruction. The viscosity effect follows in direction from the registered coffee-liquor viscosity–concentration closures (Telis-Romero; Sobolík) and is *compatible* with real machine traces — across eleven flow-controlled blooming shots on a commercial machine the post-bloom apparent resistance falls by a median factor near 2.7, of a magnitude a viscosity decline from an early first-drip concentration near 15% solids toward the dilute bulk could produce — but this is a compatibility observation, not an attribution. This contribution is bounded and early-shot-weighted — the viscosity ratio falls toward roughly 1.6 at the saturated bulk concentration and the shot-integrated Darcy effect is of order one percent — and, critically, it is *degenerate* with dissolution-linked opening: both are driven by the same falling-concentration clock, so the integrated forward trace does not separate them. Distinguishing them requires an independent time-resolved concentration measurement; the apparent-resistance decline alone is admissible for either mechanism, not diagnostic of one. A coupled calculation in which swelling and extraction share porosity actually worsens the reconstruction; that negative composition result is analyzed as a software/evidence-governance demonstration in the companion Puckworks resource paper rather than used here as a headline physical claim.
...
| Liquor-viscosity decline (concentration-linked) | Falling liquor viscosity as dissolved solids drop lowers resistance — a rising contribution, weighted to the early high-concentration interval | Static hydraulic jump; viscosity tracks the concentration clock, not the pressure step | Direction-independent — a bulk-liquid property, no deposited structure | Near-flat once the extractable inventory (hence the high-concentration liquor) is depleted; degenerate with dissolution-linked opening on this axis | No matrix signature; a liquid-property gradient tied to local concentration, not a bed deposit |
...
First-drop time constrains infiltration and dead volume, helping separate pre-saturation machine/wetting dynamics from the saturated interval analyzed here. Repeating a matched preparation under pressure control and flow control adds another intervention: a resistance increase produces falling flow under fixed pressure but rising pressure under fixed flow. The modes must be implemented with a physically consistent machine model; otherwise a nominal “control mode” label can hide different boundary conditions. One instance of this intervention already exists in logged data rather than being merely proposed: Gagné's flow-controlled blooming protocol isolates a post-wetting interval and reports a declining apparent resistance. Used with care — its per-shot mechanistic reconstruction is circular, so it constrains phenomenology, not mechanism — it supplies a real flow-control counterpart to the pressure-control trace analyzed here.
...
Second, the primary temporal comparison uses one preprocessed across-shot mean 9-bar trajectory from one campaign. Window and block sensitivity address analysis choices, not shot-to-shot, coffee-lot, preparation, operator, grinder, or apparatus replication. A second, independent apparatus does appear in the literature — eleven flow-controlled blooming shots on a commercial machine (Gagné) in which the post-bloom apparent resistance *declines* — and it exhibits the same one-trace, many-causes ambiguity from the opposite sign. It corroborates the identification problem rather than resolving any mechanism: its own per-shot yield reconstruction is endpoint-anchored and therefore circular, and (as in §5.4) the resistance decline is degenerate between a liquor-viscosity effect and a bed-compaction effect of comparable size.
