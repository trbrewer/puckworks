...
Second, the primary temporal comparison uses one 9-bar trace from one campaign. Window and block sensitivity address analysis choices, not biological or apparatus replication. A second, independent apparatus does appear in the literature — eleven flow-controlled blooming shots on a commercial machine (Gagné) in which the post-bloom apparent resistance *declines* — and it exhibits the same one-trace, many-causes ambiguity from the opposite sign. It corroborates the identification problem rather than resolving any mechanism: its own per-shot yield reconstruction is endpoint-anchored and therefore circular, and (as in §5.4) the resistance decline is degenerate between a liquor-viscosity effect and a bed-compaction effect of comparable size.
