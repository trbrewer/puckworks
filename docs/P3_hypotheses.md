...
- Hypothesis 1 also has an **unexercised structural assumption**: `brewer2026.streamtube` models the bed as parallel lognormal tubes with **no radial exchange**. Nothing on file tests when that approximation holds. The skipped `bailey2026` card (VERDICT skip; see the ROADMAP 2026-07-27 intake row) supplies a candidate harness for it — a 2D axisymmetric field-coupled Darcy solve — usable as a **structural cross-check, not as a component**: run the same prescribed K field through (i) the 2D solve and (ii) radially-decoupled columns, and quantify the Q_out and outlet-flux-variance difference as a function of ℓ_r/R. Preconditions if this is ever scheduled: the card's own K-field generation must be **ripped out and replaced** by `wadsworth2026.permeability`, because Bailey's closure is Kozeny–Carman with a 2.5×10⁻⁵ multiplicative fudge and an ad hoc fines penalty, and its spatial σ's are assumed rather than data-anchored. Its extraction and wetting layers must not be used at all. Acceptance would be a quantified difference reported into this table, **not** a validation of either representation — there is no measured 2D field to compare against.
