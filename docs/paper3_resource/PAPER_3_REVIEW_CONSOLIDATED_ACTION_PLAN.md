...
|---|---|---|
| **U1** | Name & cite Maille 2024 / Cameron 2020 / Roman-Corrochano 2017 | ✅ **CLOSED (2026-07-26).** Maille 2024 (Sheffield PhD thesis, no DOI, redacted release noted) and Roman-Corrochano 2017 (Birmingham EngD thesis, no DOI) added as references 23 and 24. |
...
| **U7** | State all protocol choices in the manuscript | ✅ **DONE (2026-07-26).** Full protocol table generated into §2.2a by `paper3.corpus`: integration horizon 400 s, τ=0 initial condition, 20 µm fine class, one- vs two-exponential comparison, per-source bath ratio/window, model-generated curve status — and, explicitly, the **coarse particle class as `not evaluated` because the source reports no radius**, with a test that fails if a value is ever substituted. |
...
| **MC1** | Decide publication genre | P1 | ✅ **DECIDED 2026-07-26 — Route A, methods/resource article**, matching the review's own P1-13 recommendation. The demonstrations, the failed composition and the defect-injection evaluation stay in the paper; in return it must meet methods-article expectations for evaluation and uncertainty, which §10 now partly discharges. Consequences accepted: figures (MC12) remain the submission blocker, and a 15–20 % trim of §2/§4 is owed. **TRIM DECISION 2026-07-26 — Tim decided NOT to trim.** The review's 15–20 % recommendation is therefore **declined, not deferred**; it should not reappear as an open item. Two facts a venue decision must account for: the manuscript is now **16,599 words**, not the 14,290 the review measured — the MC10 defect-injection section, the generated scorecard, the figures section and the scoped-evidence material added ~2.3 k — and **the venue is still undecided**, so no length limit has been checked against it. Concretely: JOSS is out on length regardless (it wants a short paper describing small software); SoftwareX caps at ~6 pages / ~3,000 words of main text with unlimited supplement, which this exceeds by ~5×; JORS and a methods journal are the realistic homes for a paper of this length. If a venue with a hard limit is later chosen, the trim returns as a *venue* requirement, not a review one. |
...
| **MC7** | Define "executable" per layer (availability matrix) | P1 | ✅ **DONE (2026-07-26).** `puckworks/paper3/availability.py` + §3.4: all eight dimensions the review listed, **215 of 216 cells DERIVED** from the registry, rights records and filesystem, machine-readable JSON, spliced with a staleness guard. Replaces "all 27 are executable" with the real counts — 27 registered/importable, 26 runnable locally, but **1 cleared for public hosted execution and 25 of 27 with no rights review on record**. A test fails if the dimensions ever stop discriminating. |
| **MC8** | Implemented capability vs architectural intent | P1 | ✅ **DONE (2026-07-26).** §3.5 implementation-status table over 8 capabilities × 7 columns, explicitly **declared with per-row evidence** rather than derived. **Arbitrary multi-stage composition and the observables stage are marked architectural intent**, with the §9 failed composition named as the worked example; a test fails if a component is ever registered in `observables` while the table calls it intent. |
...
| **MC15** | Make curated-corpus method reproducible | P1 | ✅ **DONE (2026-07-26).** New `puckworks/paper3/corpus.py` + §2.2a. **Method** recorded across all twelve aspects the review enumerated (seeds and why, sources searched incl. the two Cloudflare-blocked publishers, rolling search dates, exploratory strings, backward/forward tracing, inclusion/exclusion rules, derivative and duplicate handling, English-only limit, rights-blocked handling, project-vs-published separation by `provenance_class`, and how the search was steered by missing stage contracts). It **declines to claim systematic** and a test fails if the word 'exhaustive' appears. **Denominators** are DERIVED from the live registry and manifest and make the misreading impossible: the **27 components come from 19 source publications** (11 with a DOI) — 1.42 components per publication — and **only 9 of 27 carry evidence beyond a reconstruction of their own source's output**. Also reported: 40 dataset sources, 107 manifest records, 96 cards, counts by provenance class / evidence relation / execution role, 15 calibration-only, 3 reference-only, 1 rights-blocked. Spliced with a staleness guard; prose counts are test-bound to the generated ones. 12 tests. |
