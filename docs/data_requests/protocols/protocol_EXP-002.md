# Protocol pack — EXP-002: Initially dry puck infiltration and physical first drip

**Scientific decision.** Does the parameter-free Foster front bracket the physically observed first drip, kept distinct from first modeled solute arrival?

**Models / gates affected.** `foster2025.infiltration` → `gate_infiltration_triangle`.

**Apparatus.** inlet-pressure log; outlet mass/flow; an INDEPENDENT physical first-drip detector. Record the full apparatus in `../templates/apparatus.yml`.
...
**Experimental factors & controls.** initially dry bed; record bed dimensions, dry mass, moisture, porosity; the first-drip detector must be independent of the model.
...
recorded tolerance; (for EXP-002 chemistry) summed fraction mass matches cup mass within tolerance and
