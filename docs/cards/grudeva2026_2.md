...
- Reduced-model Python solver is published: github.com/YoanaGrudeva/espresso-model (SI ref [1]). **Rights (updated 2026-08-14):** the repo still declares **no SPDX license**, but the upstream author, Dr. Yoana Grudeva, gave the maintainer **direct written permission** to use and share its code and data — recorded as `PERMISSION_DOCUMENTED` in `puckworks.rights` (issue [#73](https://github.com/trbrewer/puckworks/issues/73); sanitized record `docs/permissions/grudeva2025.md`; packaging scope `THIRD_PARTY_NOTICES.md`). That is **permission, not a licence**: the upstream material is not MIT/CC-BY/public-domain and is not relicensed. It removes the rights bar; it does **not** decide this card's implementation approach — the equations are separately CC-BY-4.0 (the EJAM article is open access), and #67's scoped plan remains an independent re-implementation from the published equations with the solver's outputs as external parity targets. Full-model MATLAB code is not stated to be published. *Superseded (historical): until 2026-08-14 this caveat read "all-rights-reserved; do not copy/vendor/transcribe the code" — correct when written, since no permission was then on record.*
...
- **cameron2020.extraction_bdf (extraction, runtime):** direct competitor and lineal descendant (Foster is senior author on both; model built explicitly on Cameron's homogenisation). Grudeva adds the dry-bed infiltration phase, boulder refill sink at the front, and an asymptotic reduction; Cameron has the stronger experimental grounding (SI Tables S1–S5 flux/microstructure + Fig. 5 EY curve) and a validated implementation. They should coexist: Cameron as the gated saturated-bed workhorse, Grudeva as the infiltration-aware variant whose early-time transient Cameron cannot produce.
