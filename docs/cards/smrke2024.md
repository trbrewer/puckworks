...
- **Validation target for the extraction chain:** the Fig. 3 EY(t) master curve at fixed 20 g/40 g/9 bar is a clean gate for cameron2020.extraction_bdf-style predictions: at fixed brew ratio, EY should collapse to a function of shot time across PSD manipulations. Also encodes the turbo-efficiency constraint (17–18% EY at <15 s).
...
Effort S: digitize Figs. 2 and 3 into two CSVs with fines-spike flags and the 100/120 µm definition note; record PLSR sign structure and group m/z lists in metadata. No gate against the source is possible (no published coefficients or error statistics to reproduce). Gate design for consumers: an extraction-stage configuration at 20 g/40 g/9 bar must land its (EY, t_shot) pairs on the Fig. 3 curve within its scatter (~±0.5 pt EY at fixed t) across a Q100µm 15–31% sweep, and must show EY ≥ 0.8 × EY_max by t = 15 s. Dependencies: supplement acquisition (Fig. S1) before closing the data-transcription scope; optional author request for raw tables.

VERDICT: data-only — no recoverable model (all regressions figure-only, coefficients unpublished), but a clean fixed-recipe EY(t) master curve spanning a controlled fines sweep, giving the registry its first direct gate that fines act via permeability alone within the tested envelope — effort S.
