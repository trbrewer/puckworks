...
Almost every model of coffee extraction agrees on a shape: a fast burst, then a long slow tail. It is the most recognizable curve in the field, and it is tempting to think the fast and slow "speeds" one model reports can be carried over to another. The registry checked, and they can't. Take one lab model's fast/slow numbers and look for them in a second model's brewing curve: the two speeds **merge into one** — that model's flowing puck really only has a single pace, and the "fast" part has nowhere to live. Look in a third model, a genuinely well-stirred one, and you *do* get a fast and a slow number — but they're the same every time regardless of grind, because they're just the early and late parts of **one** process (a single grain slowly giving up its contents), not two real populations; and they land about a hundred times off the first model's numbers anyway. So the same familiar curve is produced by three different stories: fine particles versus coarse in one, big molecules versus small in another, and how fast water flows through in the third. The data-literacy punchline is the sequel to §7.13: *even when the picture matches, the numbers behind it may not transfer.* A curve that fits beautifully is **not** proof that two models are measuring the same thing — pull the fitted "speeds" out of one and into another and you can be off by orders of magnitude. Build a triptych: one identical-looking "fast then slow" curve, three captions naming the three different mechanisms underneath. Evidence tier is **qualitative** — these are model-versus-model curve fits (two of the curves are model-generated, one on a single particle size), not measurements, and **not** a validation of any model. Pairs with §7.13 and §7.2, and seeds the PV-15 "which experiment separates them" recommender.
