...
**Delivery status (2026-07-18).** The public-results layer (PV-00) is built. Three static interactives are **live** on GitHub Pages: **PV-03 "The Cup Hides the Clock"** (<https://trbrewer.github.io/puckworks/flat-valley/>), **PV-05 "More Physics Made It Worse"** (<https://trbrewer.github.io/puckworks/model-composition/>), and **PV-04 "How We Falsified Our Own Espresso Headline"** (<https://trbrewer.github.io/puckworks/analysis-autopsy/>) — PV-04 with replicate-level EY points, per-cell descriptive SDs, and per-panel evidence partitions, verified live (issue #62). The raw-replicate-points requirement and evidence-safe PV-04 language below are retained. No next public-value product lane has been authoritatively selected — selection is pending. The Guided Espresso Pull shipped in v0.3.0 (signed-out human acceptance pending, issue #48).
...
### 7.12 “Two respected permeability models disagree by ~20× exactly where neither was tested”

Two published closures predict how hard water pushes through the coffee puck. One (a percolation model) was validated on *loose* beds; the other (a modified Kozeny–Carman fit) was measured on a *tamped* bed — the espresso-relevant regime. Evaluated on the same tamped operating points, they disagree by roughly **13–31× (median ~20×)**, and the gap *grows* as the bed is packed tighter, i.e. exactly below the packing density where the first model was ever checked. The data-literacy surprise is that this is not sloppiness on either side — both are careful, both are "right" in their own tested range — and the honest registry move is to **show the disagreement, mark the untested zone, and name the one measurement that settles it**, rather than split the difference. Build a visual of the two curves fanning apart as the puck tightens, with a shaded "neither model was tested here" band over the espresso regime. Evidence tier is **verification/discrimination** — a model-versus-model comparison on a *single* dark-roast material, with the tamped closure post-fit and the answer sensitive to the assumed liquid viscosity (the fitted constant shifts ~7.5→10 under the registry's espresso-liquor viscosity); it must **not** be read as a validated permeability law. Discriminating measurement: an independent, multi-material tamped-bed permeability set with a stated viscosity convention. A concrete seed for the PV-15 "which experiment separates them" recommender; pairs with §7.7 (dimensionless numbers are not interchangeable) and PV-08.

### 7.13 “Two coffee models both track a ‘fast-extracting fraction’ — but they don’t mean the same thing”

Espresso extraction is often split into a quick "washing" phase and a slow diffusion phase, and two independent models each carry a number for *how much* of the coffee extracts fast. It is tempting to treat those two numbers as the same thing — they have the same name and both are fractions between 0 and 1. They are not. One model builds its fast fraction from particle geometry (all the finest particles, plus a thin fast-extracting shell on every coarse grain); the other counts only a distinct class of very fine particles. Run the first model's recipe on the second model's *measured* grind and the two fast fractions **both fall as the grind gets coarser — they agree on the direction** — but they differ by **five- to nine-fold** in size. The data-literacy point is the cleanest possible version of this project's core lesson: *a shared name is not a shared quantity.* Averaging them, or plugging one into the other's equation, would silently inject a several-fold error — the same trap as the "three different saturation concentrations" card (§7.2). Build a two-panel visual: same word, two definitions, two very different bars — with a caption on what each actually counts. Evidence tier is **verification/discrimination** — a model-versus-model comparison on one material and one grind family, and an *extrapolation* of the geometric closure onto a finer espresso grind than it was built for; it must **not** be read as one model validating the other. This is a definitional gap, so unlike §7.12 there is no single experiment that "settles" it — the fix is to keep the two quantities labelled and never equated. Pairs with §7.2 and feeds the PV-08 "Puck Court" evidence dashboard.

### 7.14 “‘Fast then slow’ shows up in three coffee models — for three different reasons”

Almost every model of coffee extraction agrees on a shape: a fast burst, then a long slow tail. It is the most recognizable curve in the field, and it is tempting to think the fast and slow "speeds" one model reports can be carried over to another. The registry checked, and mostly they can't. Take one lab model's fast/slow numbers and look for them in a second model's brewing curve: for the finer grinds the two speeds **merge into one** — that model's flowing puck really shows a single pace, and the "fast" number has nowhere to land; for the coarsest grind it does keep two separate numbers, but neither matches the first model's fast speed. Look in a third model, a genuinely well-stirred one, and you *do* get a fast and a slow number — and here's the subtle part: the *proportions* between them stay the same whatever the grind (they're just the early and late parts of **one** slow-release process, not two real populations), even though the actual speeds shift with particle size. For the one particle size on file they land far from the first model's numbers — though a coarser size, which the paper never reported, could move them closer. So the same familiar curve is produced by three different stories: fine particles versus coarse in one, big molecules versus small in another, and an aggregate flowing-bed response in the third. The data-literacy punchline is the sequel to §7.13: *even when the picture matches, the numbers behind it may not transfer.* A curve that fits beautifully is **not** proof that two models are measuring the same thing. Build a triptych: one identical-looking "fast then slow" curve, three captions naming the three different mechanisms underneath. Evidence tier is **qualitative** — these are model-versus-model curve fits (both compared curves are model-generated, one on a single fine particle size), not measurements, and **not** a validation of any model. Pairs with §7.13 and §7.2, and seeds the PV-15 "which experiment separates them" recommender.
