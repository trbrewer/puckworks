...
| 2026-07-26 | Paper 3 frozen-release build | **The last non-DOI submission blocker closed.** (1) Figures, five source-data CSVs and `ALT_TEXT.md` added to the deterministic archive — **147 → 149 members**, byte-identical across two builds, still verifiable without the source checkout. (2) New **`release` verb** (`python -m puckworks.paper3.build release`): clean tree required, **freshness proven by RECOMPUTATION** (every generated artifact regenerates byte-identically; archive hashes the same twice), writes `docs/reproducibility/paper3_release_manifest.json`. (3) `bundle_contents()` delegates to the archive with a floor list asserted so the two cannot drift — **immediately caught two evidence files the archive was missing**. (4) `REPRODUCIBILITY.md`: determinism / freshness / **correctness** kept apart, correctness explicitly **not** certified by either of the others, plus "What is NOT yet reproducible". **Two self-corrections, both tested:** "commit-equality is structurally unsatisfiable" holds only **in-tree** (`tools/prepare_paper_release.py` achieves it out-of-tree for Papers 1–2, and does **not** cover Paper 3); and the first `requirements-papers.lock` named a **per-session scratch path** in 12 lines, so `docs/reproducibility/constraints-papers.txt` is now committed, the lock regenerated, and a guard proven to fail on the original. `REPRODUCIBILITY.md` now distinguishes the three lock artifacts (direct-only / transitive / `pip freeze` record). **Running the gate then found three more defects.** (a) It **failed because it ran** — `release` writes its report into the tree, dirtying it for the next run; the report's path is now excluded, and only it. (b) The first fix was **incomplete**: `create_archive` had its own independent `git status` check, so the second run still died, with an **unhandled `RuntimeError`** and no manifest at all. `archive.dirty_paths()` is now the one definition; a refusal is reported as `archive_refused:<why>`; the porcelain parser is tested against `??`/`MM`/`R old -> new`/` D`. (c) The **archive hash depends on the commit**, not only on contents (mtimes come from committer time) — verified both ways and documented, since a reader reproducing a published hash would otherwise conclude the archive was non-deterministic. Gate **green on a clean tree, twice, identical hash**; manifest committed. 28 tests. **Remaining: the archival DOI** — a human deposit decision, not a build gap. one transitive lock now covers all three papers. |
| 2026-07-26 | one transitive lock for all three papers | **Closes the pre-archival requirement in `HANDOFF_EXECUTION_SUMMARY_2026-07-13.md`** (that `requirements-paper-release.lock` is direct-only and a full transitive resolution was owed). The task was "transitive locks for Papers 1–2 mirroring Paper 3's"; the correct outcome turned out to be **one lock, not three**. All three papers resolve from the same `pyproject.toml` with the same `figures` extra, and every producing module — `figures_paper_a`, `paper_a.build`, `paper_b.build`, `figures_paper3`, `paper3.build`, `paper3.archive` — imports **nothing beyond numpy, scipy and matplotlib** (verified by AST). Three per-paper locks would be three byte-identical files free to drift. `requirements-paper3.lock` → **`requirements-papers.lock`** (+ `constraints-papers.txt`); `requirements-paper-release.lock` retained because `RELEASE_RUNBOOK.md` names it, now marked as the narrower of two. A test asserts the shared import set, so if any pipeline grows a fourth dependency the single lock stops being adequate **loudly** — proven non-vacuous against an injected `import pandas`. 10 lock tests. |
| 2026-07-26 | Paper 2 item 4.13 + Paper 3 MC7/MC8 | **4.13 — the claim map now covers EVERY number in the Paper 2 body: 18 claims → 118, unaccounted 150 → 0.** New `puckworks/paper_b/claim_coverage.py` audits all 392 numerals in the manuscript body and forces each into a disposition (`producer` 137 / `config` 159 / `dataset` 17 / `derived` 5, recomputed not waved through / `structural` 74). The shot-level layer, the held-out comparator, the residual diagnostics and the 33-cell per-pressure table are now bundled and bound — the per-pressure claims are **expanded from the producer**, since 33 hand-transcribed cells is exactly how Table 3's `rc3b` column went stale in every row. **Three unbacked numbers found and fixed.** The six recorded-pressure values in §5.2 had been transcribed from a reviewer's independent table with **no producer of ours**; the new `recorded_pressure_robustness()` reproduces them exactly (0.647696→0.646846, 0.115769→0.116443). The claimed pressure deficit of **"up to 0.61 bar"** and the **"mean 8.71 bar"** at 9 bar are **not reproducible under any natural definition** (candidates: 0.508 / 0.540 / 0.673 bar, and 8.737 / 8.746 / 8.768 bar) — both replaced with producer-backed values under a declared definition via `nominal_vs_recorded_pressure()`. A macro-mean printed as **0.335** contradicted Table 3's **0.334** for the same quantity; corrected. Also fixed `_get`, which split dotted paths naively and reported every per-pressure cell MISSING. **MC7/MC8** — new `puckworks/paper3/availability.py`: an 8-dimension availability matrix, **215 of 216 cells derived**, spliced into §3.4/§3.5 with a staleness guard. It replaces "all 27 components are executable" with the real counts: 27 registered and importable, 26 runnable locally, but **only 1 cleared for public hosted execution and 25 of 27 with no rights review on record**. MC8's implementation-status table is explicitly **declared** with per-row evidence and marks arbitrary composition and the observables stage as **architectural intent**. A first pass wrongly reported 3 calibration components as unimportable because their `module` is a `module:attribute` dataset pointer — fixed and pinned. 26 tests; CI wired for both verifiers. |
| 2026-07-26 | Paper 3 MC15 + U7 + U1 | **MC15 — the curated corpus is now documented and its denominators derived.** New `puckworks/paper3/corpus.py` + §2.2a. The method covers all twelve aspects the review enumerated and **declines to claim systematic** (a test fails on the word "exhaustive"). The denominators make the misreading the review warned about impossible: **27 components from 19 source publications** (11 with a DOI) — 1.42 per publication — and **only 9 of 27 carry evidence beyond a reconstruction of their own source's output**. Also 40 dataset sources, 107 manifest records, 96 cards, and counts by provenance class, evidence relation and execution role. **U7** — the full protocol table lands in the same section, including the coarse particle class recorded as `not evaluated` because the source reports no radius (test-bound so no value can be substituted). **U1** — Maille 2024 and Roman-Corrochano 2017 added to the reference list as 23/24, both flagged as thesis, no DOI. Spliced with staleness guards; prose counts test-bound to generated ones; verifier wired into CI. 12 tests. |
