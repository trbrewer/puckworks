...
The *two-regime decomposition itself*—not just its fast fraction, but the fast-then-slow structure—is the same hazard one level up. Maille’s 2024 batch-extraction model explicitly parameterizes a fast and a slow phase; Cameron et al.’s 2020 flowing-bed model and Roman-Corrochano’s 2017 stirred-vessel model produce extraction curves that *can also be described* in fast/slow language, and it is tempting to treat one model’s fitted fast and slow timescales as transferable to another. We tested this by fitting Maille’s empirical bi-exponential form (with the hydration delay fixed to zero) to *model-generated* curves from the other two—these are qualitative model-to-model probes, not validation against independent measurements. Fitting it to a Cameron curve—run to exhaustion at ~400 s, far past that model’s ~30 s validated recipe—no fitted fast constant falls within Maille’s pooled 2.2–19.1 s fast range; three of the four grinder settings collapse to a single-exponential-like response (the two fitted constants coincide and a second exponential is non-identifiable), while the coarsest returns two *separated* constants (≈ 23.6 and 40.0 s) that would require a formal one- versus two-timescale adjudication. Fitting the same form to a Roman-Corrochano stirred-vessel curve, the fitted *weight* and *slow/fast ratio* are invariant across grinds while the *absolute* constants scale with the diffusion time and vary with grind; for the one particle class available in the repository (a ~20 µm-radius fine class—the coarse-class size is not published, and diffusion time scales as radius squared) both constants are sub-second and below Maille’s ranges, a configuration-specific rather than universal result. These comparisons show that a shared bi-exponential form is a useful *descriptive* basis but not a shared *physical* contract: Maille’s constants parameterize a geometric two-pool split, Roman-Corrochano’s summarize one physical diffusion process in a selected particle/species configuration, and Cameron’s summarize an aggregate flowing-bed response. A high goodness-of-fit does not make the coefficients commensurate—transferring a fitted fast/slow timescale between models as though it were the same physical constant is the same category error as equating the fast fractions—so the registry keeps each fitted timescale labelled by its model of origin unless observable identity, mechanism, estimation protocol, and predictive transfer are separately established.
