...
The *two-regime decomposition itself*—not just its fast fraction, but the fast-then-slow structure—is the same hazard one level up. Three extraction models all resolve a quick phase and a slow phase, and it is tempting to treat one model’s fast and slow timescales as transferable to another. The registry finds they are not. Fitting one batch model’s empirical bi-exponential form to a second model’s simulated extraction curve, the fast and slow constants collapse onto a single timescale: that model’s flowing bed is effectively one-regime, and the batch model’s fast timescale has no counterpart in it. Fitting the same form to a third model’s genuinely well-mixed diffusion curve does produce two constants—the fit is good—but they are grind-independent and hold a fixed ratio, because they are the intrinsic short- and long-time signature of a *single* diffusion mode, not two physical pools; and at that model’s own particle size they fall two orders of magnitude away from the batch model’s measured range. So a shared bi-exponential form flatters the eye with a high goodness-of-fit while its parameters mean three different things: a two-pool geometric split in one model, one diffusion mode’s transient in another, a flow-limited single scale in the third. The lesson for model composition is that a good fit is not evidence of a shared quantity—reading fitted fast/slow timescales as portable physical constants is the same category error as equating the fast fractions, and the registry’s response is again to keep the constructs labelled by their model of origin rather than pooled under a common name.
