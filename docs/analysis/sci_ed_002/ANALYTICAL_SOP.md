...
The 1%/two-consecutive/eight-cycle combination is preserved as the R0 proposed rule, not a commissioning-ready rule. R1 provenance audit found no direct requirement or valid quantitative derivation for those three numbers. Commissioning is blocked until a separately reviewed numeric rule is defensibly frozen; see `r1/REFERENCE_EXTRACTABILITY_STOPPING_RULE_PROVENANCE.json`. Public literature supports hot-water extraction and simultaneous HPLC measurement but not this endpoint.
