...
**More public stories:** [See why espresso model components do not simply add up](https://trbrewer.github.io/puckworks/model-composition/) — a static, generated interactive showing how adding a swelling branch to an extraction model, through one shared porosity state, makes the prediction *worse* than a flat baseline (it rejects one tested composition, not swelling in general).
