...
| Grind | `maille2024.phi_closure` | Calibration / comparison | Predicts from a measured particle size distribution what fraction of the soluble material releases fast — fines plus a thin shell on each coarse particle. Instrument-specific (186 µm cut); coarse grinds only, so espresso is an extrapolation. ([card](docs/cards/maille2024.md)) |
...
| Extraction | `grudeva2025.reduced` | Registered scientific check — no common-scenario adapter yet | A reduced asymptotic extraction model with a moving wetting front and fine/coarse particle families (saturated-fines regime; prescribed flow). Its code is a faithful port of the authors' solver, used under **Dr. Yoana Grudeva's direct written permission, 2026-08-14**, covering code and data ([#73](https://github.com/trbrewer/puckworks/issues/73), `PERMISSION_DOCUMENTED`); **no SPDX licence** — not MIT/CC-BY/public-domain ([permission](docs/permissions/grudeva2025.md), [notices](THIRD_PARTY_NOTICES.md)). Runs its **registered gates**; not a common-scenario lens — its grain-volume basis has no tested conversion to bed-volume (technical, not rights). Evidence unchanged. ([card](docs/cards/grudeva2025.md)) |
...
| Extraction | `maille2024.two_regime` | Calibration / comparison | Early-time release of five named compounds in a stirred batch vessel, as a fast phase plus a slow phase. The best sub-30-second per-compound data here, but with no bed, pressure or flow, and no yield or strength output. ([card](docs/cards/maille2024.md)) |
...
No registered component is currently validated as a universal whole-process coupled espresso model. Each
runs, or constrains, its own stage; the Guided Pull executes only the Cameron extraction chain above. The
**Espresso Model Relay** is a separate, assumption-rich *product* demonstration that illustrates selected
hand-offs between components (direct links, documented adapters, and explicit assumptions) without claiming
whole-chain validation.
<!-- puckworks-model-map:end -->

## What you can run today

Five public paths (no local setup required):
...
- **Run the Full Laboratory Tour in your browser** — [🧪 Open the Guided Pull Laboratory in Colab](https://colab.research.google.com/github/trbrewer/puckworks/blob/main/notebooks/guided_pull_laboratory_colab.ipynb): a short **form** plus one **▶ Run the Laboratory** button — no terminal, no code to type. The default **Full Laboratory Tour** resolves *all 27* registered components and runs the *26* that are scientifically and legally available — one common-scenario run of your recipe (Cameron), four native reference cases, and twenty-one registered scientific checks — grouped by espresso stage with a plain-language badge on every card (other modes: *Quick Tour*, *Your Shot Only*, *Model Library Only*). Outputs are **never** averaged, ranked, or overlaid: a native case is not a prediction of your shot, and a gate pass is not experimental validation. It runs **privately** in your own Colab runtime (`LOCAL_PRIVATE`; not a public-hosting clearance), installs a commit-pinned development preview (`0.4.0.dev0`), runs `grudeva2025.reduced` only as its registered gates (#73), and is **not** a digital twin, optimizer, or taste predictor. See [`docs/GUIDED_PULL_LABORATORY.md`](docs/GUIDED_PULL_LABORATORY.md).
- **Follow one illustrative pull through linked models** — [🔗 Open the Espresso Model Relay in Colab](https://colab.research.google.com/github/trbrewer/puckworks/blob/main/notebooks/illustrative_linked_pull_colab.ipynb): one hypothetical espresso pull, passed from model to model. At each station it shows what the model calculates, what is handed forward, what had to be assumed, and what the result might mean for the cup. An **assumption-rich educational relay, not a validated coupled simulation** — a *separate* experience from the Full Laboratory Tour (which is unchanged). The default fast run authoritatively executes 20 components with 10 explicit, serialized cross-model hand-offs (each labelled direct / documented-adapter / illustrative-assumption), a first-class assumption ledger, Cameron as a baseline (not the whole story), and a dashboard that **never** averages branches and produces no taste or confidence score. Runs **privately** (`LOCAL_PRIVATE`), gives `grudeva2025.reduced` zero calls — no compatible hand-off, not a rights block (#73) — and is **not** a digital twin, optimizer, or taste predictor. See [`docs/ILLUSTRATIVE_LINKED_PULL.md`](docs/ILLUSTRATIVE_LINKED_PULL.md).
...
- [See how we falsified our own espresso headline](https://trbrewer.github.io/puckworks/analysis-autopsy/) — a static, generated analysis autopsy. An attractive interior "fine-grind" extraction-yield maximum weakened once the target was rebuilt on one coherent TDS basis and the raw replicate points were plotted: the corrected cell means are ordered with the middle dial below the coarse dial (no interior maximum), while a model can still draw such a feature without identifying its cause. Every number is generated and bound to the CC-BY Schmieder source. These are grinder-**dial** positions on one machine and coffee — a dial is not a particle size and does not describe a universal grinder behaviour.
...
| CT infiltration and machine mode (Foster 2025) | Time-resolved micro-CT wetting fronts, headspace, and flow curves | Checks the infiltration front and the machine-mode delivery model | Post-fit, same-campaign CT observations / verification of fitted trajectories |
...
| Validation gates | 66 total — 65 PASS, 1 ACKNOWLEDGED_EXCEPTION (passed under the documented gate policy) |
...
19. Perticarini, A. "Predictive Models in Espresso Coffee Percolation." Ph.D. thesis, Universita degli Studi di Camerino, School of Advanced Studies, deposited 28 Feb 2024. No DOI. Related cards: [perticarini2024](docs/cards/perticarini2024.md), [perticarini2024_3d](docs/cards/perticarini2024_3d.md).
20. Roman-Corrochano, B. "Advancing the engineering understanding of coffee extraction." EngD thesis, University of Birmingham, 2017. No DOI is recorded in the model card (University of Birmingham e-theses repository). Related cards: [romancorrochano2017_extraction](docs/cards/romancorrochano2017_extraction.md), [romancorrochano2017_permeability](docs/cards/romancorrochano2017_permeability.md).
21. Schmieder, F.; Pannusch, V. B.; Vannieuwenhuyse, A.; Briesen, H.; Minceva, M. "Influence of flow rate, particle size, and temperature on espresso extraction kinetics." *Foods* 12 (2023): 2871. [https://doi.org/10.3390/foods12152871](https://doi.org/10.3390/foods12152871). Related cards: [schmieder2023](docs/cards/schmieder2023.md).
22. Schulman, J. "How filter baskets affect espresso taste and barista technique." Home-Barista.com study, 2011. No DOI is recorded in the model card. Related cards: [schulman2011](docs/cards/schulman2011.md), [schulman2011_baskets](docs/cards/schulman2011_baskets.md).
23. Telis-Romero, J.; Gabas, A. L.; Polizelli, M. A.; Telis, V. R. N. "Temperature and water content influence on thermophysical properties of coffee extract." *International Journal of Food Properties* 3(3) (2000): 375–384. [https://doi.org/10.1080/10942910009524643](https://doi.org/10.1080/10942910009524643). Related cards: [telisromero2000](docs/cards/telisromero2000.md).
24. Telis-Romero, J.; Cabral, R. A. F.; Gabas, A. L.; Telis, V. R. N. "Rheological properties and fluid dynamics of coffee extract." *Journal of Food Process Engineering* 24 (2001): 217–230. [https://doi.org/10.1111/j.1745-4530.2001.tb00542.x](https://doi.org/10.1111/j.1745-4530.2001.tb00542.x). Related cards: [telisromero2001](docs/cards/telisromero2001.md).
25. Wadsworth et al. *Royal Society Open Science* 13 (2026): 252031. [https://doi.org/10.1098/rsos.252031](https://doi.org/10.1098/rsos.252031). Title and author names are not recorded in the model card. Related cards: [wadsworth2026](docs/cards/wadsworth2026.md), [wadsworth2026_grindmap](docs/cards/wadsworth2026_grindmap.md), [wadsworth2026_inertial](docs/cards/wadsworth2026_inertial.md).
26. Waszkiewicz, S.; Myck, M.; Białas, M.; Puciata-Mroczynska, M.; Dzikowski, M.; Szymczak, P.; Lisicki, M. "Under pressure: poroelastic regulation of flow in espresso brewing." arXiv:2512.21528 (2025); code and data on Zenodo. [https://arxiv.org/abs/2512.21528](https://arxiv.org/abs/2512.21528). Related cards: [waszkiewicz2025](docs/cards/waszkiewicz2025.md).
27. Yasuda, H.; Katsuragi, H.; Katsura, M. "Liquid water transport model in hydrophilic granular." arXiv:2501.13361 (2025). [https://arxiv.org/abs/2501.13361](https://arxiv.org/abs/2501.13361). Related cards: [g1_glassbead_analog](docs/cards/g1_glassbead_analog.md).
28. Moroney, K. M.; O'Connell, K.; Meikle-Janney, T.; O'Brien, S. B. G.; Walker, G. M.; Lee, W. T. "Analysing extraction uniformity from porous coffee beds using mathematical modelling and computational fluid dynamics approaches." *PLoS ONE* 14(7) (2019): e0219906. [https://doi.org/10.1371/journal.pone.0219906](https://doi.org/10.1371/journal.pone.0219906). Author-confirmed errata via Cooper, M. "Modeling Extraction," Quantitative Café (2021), [https://quantitativecafe.com/2021/11/14/modeling-extraction/](https://quantitativecafe.com/2021/11/14/modeling-extraction/). Related cards: [moroney2019](docs/cards/moroney2019.md), [cooper2021](docs/cards/cooper2021.md).
29. Ribes, S. "Radial uniformity of espresso extractions." Slide deck (Decent community), 2020. No DOI is recorded in the model card. Related cards: [ribes2020](docs/cards/ribes2020.md).
30. Ribes, S. "Espresso extraction radial uniformity — BPLUS contact screen." Slide deck (Decent community), 2021. No DOI is recorded in the model card. Related cards: [ribes2021](docs/cards/ribes2021.md).
31. Roman-Corrochano, B.; Melrose, J. R.; Bentley, A. C.; Fryer, P. J.; Bakalis, S. "A new methodology to estimate the steady-state permeability of roast and ground coffee in packed beds." *Journal of Food Engineering* 150 (2015): 106–116. No DOI is recorded in the model card. Related cards: [romancorrochano2015](docs/cards/romancorrochano2015.md).
32. Gagné, J. "A comparison between standard and low-fines espresso shots." Coffee Ad Astra (blog / Patreon), 2021. No DOI is recorded in the model card. Related cards: [gagne2021](docs/cards/gagne2021.md).
33. Khamitova, G. "Advanced experimental and analytical study for the optimization of the espresso coffee extraction." PhD dissertation, University of Camerino, 2020. No DOI is recorded in the model card. Related cards: [khamitova2020](docs/cards/khamitova2020.md).
34. Smrke, S.; Eiermann, A.; Yeretzian, C. "The role of fines in espresso extraction dynamics." *Scientific Reports* 14 (2024): 5612. [https://doi.org/10.1038/s41598-024-55831-x](https://doi.org/10.1038/s41598-024-55831-x). Related cards: [smrke2024](docs/cards/smrke2024.md).
35. Sobolík, V.; Žitný, R.; Tovcigrecko, V.; Delgado, M.; Allaf, K. "Viscosity and electrical conductivity of concentrated solutions of soluble coffee." *Journal of Food Engineering* 51 (2002): 93–98. [https://doi.org/10.1016/S0260-8774(01)00042-5](https://doi.org/10.1016/S0260-8774(01)00042-5). Related cards: [sobolik2002](docs/cards/sobolik2002.md).
36. Moroney, K.; Lee, W.; O'Brien, S.; Suijver, F.; Marra, J. "Modelling of coffee extraction during brewing using multiscale methods: an experimentally validated model." *Chemical Engineering Science* 137 (2015): 216–234. [https://doi.org/10.1016/j.ces.2015.06.003](https://doi.org/10.1016/j.ces.2015.06.003). Related cards: [moroney2015](docs/cards/moroney2015.md).
...
Puckworks is released under the [MIT License](LICENSE), covering Puckworks-authored material.
Third-party material on another basis is in [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md) — today
the Grudeva code and derived vial statistics, under **direct written permission** (2026-08-14), **not**
an SPDX licence and **not** MIT. It is independently developed and is not affiliated with or sponsored
by any employer or company. Contact: t_r_brewer@hotmail.com.
